<?php
    ...
    class MahasiswaController{
        ...
        public function delete() {
            $id = $_POST['delete'];
            $data_mhs = file_get_contents('./data/mahasiswa.json');
            $data_mhs = json_decode($data_mhs);
            
            $arr_mhs = array_column($data_mhs, 'id');
            $index = array_search($id, $arr_mhs);
              
            array_splice($data_mhs, $index, 1);
            
            $data_mhs = json_encode($data_mhs, JSON_PRETTY_PRINT);
            file_put_contents('./data/mahasiswa.json', $data_mhs);
            header('Location: '. 'http://' . $_SERVER['HTTP_HOST']);
            exit;
